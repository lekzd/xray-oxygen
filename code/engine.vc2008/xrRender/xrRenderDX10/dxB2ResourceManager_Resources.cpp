...
#include "../xrRenderDX10/dxB2ConstantBuffer.h"
...
#ifdef USE_DX12
ID3D12Resource* CResourceManager::LoadTextureSynchronize(D3D12_RESOURCE_DIMENSION dimension, DWORD dwWidth, DWORD dwHeight, DXGI_FORMAT format, void* pData, DWORD DataSize, DWORD Pitch /*= 0*/)
{
    //#TODO!
    ...
    textureDesc2.Flags = D3D12_RESOURCE_FLAG_NONE;

    HW.pDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
    ...
    //Create a temp resource, to upload data for newer resource

    ComPtr < ID3D12Resource > IntermediateData;

    const DWORD SubresourceCounts = textureDesc2.MipLevels * textureDesc2.DepthOrArraySize;
    DWORD64 UploadResourceData = GetRequiredIntermediateSize(Result.Get(), 0, SubresourceCounts);

    HW.pDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
        D3D12_HEAP_FLAG_NONE,
        &CD3DX12_RESOURCE_DESC::Buffer(UploadResourceData),
        D3D12_RESOURCE_STATE_GENERIC_READ,
        nullptr,
        IID_PPV_ARGS(&IntermediateData));

    D3D12_SUBRESOURCE_DATA TextureSubData;
    TextureSubData.pData = pData;
    TextureSubData.SlicePitch = DataSize;
    TextureSubData.RowPitch = Pitch;

    // Use main command list, because we want access to newer texture once we exit this function
    UpdateSubresources(HW.pCommandList.Get(), Result.Get(), IntermediateData.Get(), 0, 0, SubresourceCounts, &TextureSubData);
    HW.pCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(Result.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));

    return Result.Detach();
}

ID3D12Resource* CResourceManager::LoadTextureSynchronize(DirectX::TexMetadata& texInfo, DirectX::ScratchImage& image)
{
    ComPtr < ID3D12Resource > FreshTex;
    CHK_DX(DirectX::CreateTexture(HW.pDevice.Get(), texInfo, &FreshTex));

    ComPtr < ID3D12Resource > IntermediateData;

    const DWORD SubresourceCounts = texInfo.mipLevels * texInfo.arraySize;
    DWORD64 UploadResourceData = GetRequiredIntermediateSize(FreshTex.Get(), 0, SubresourceCounts);
    ...
    CHK_DX(DirectX::PrepareUpload(HW.pDevice.Get(), image.GetImages(), image.GetImageCount(), texInfo, FluidHHGGImgSubresource));

    // Use main command list, because we want access to newer texture once we exit this function
    UpdateSubresources(HW.pCommandList.Get(), FreshTex.Get(), IntermediateData.Get(), 0, 0, FluidHHGGImgSubresource.size(), FluidHHGGImgSubresource.data());
    HW.pCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(FreshTex.Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));

    return FreshTex.Detach();
}

void CResourceManager::LoadTextureAsync()
{
    //#TODO!
}
#endif