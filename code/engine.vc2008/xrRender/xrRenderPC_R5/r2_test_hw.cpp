...
#endif
#ifdef USE_DX12
typedef HRESULT(__stdcall *FuncPtrD3D12CreateDeviceAndSwapChain)(
	IDXGIAdapter* pAdapter,
	D3D_DRIVER_TYPE DriverType,
	HMODULE Software,
	UINT Flags,
	...
	D3D_FEATURE_LEVEL* pFeatureLevel,
	ID3DDeviceContext** ppImmediateContext);
#endif
#ifdef USE_DX11
bool TestDX11Present()
{
	...
	if (!hD3D11)
	{
		Msg("* DX11: failed to load d3d12.dll");
		return false;
	}
	
	FuncPtrD3D11CreateDeviceAndSwapChain pD3D11CreateDeviceAndSwapChain = 
		(FuncPtrD3D11CreateDeviceAndSwapChain)GetProcAddress(hD3D11, "D3D12CreateDeviceAndSwapChain");
...
bool TestDX12Present()
{
	...
	FuncPtrD3D12CreateDeviceAndSwapChain pD3D12CreateDeviceAndSwapChain =
		(FuncPtrD3D12CreateDeviceAndSwapChain)GetProcAddress(hD3D12, "D3D12CreateDeviceAndSwapChain");

	if (!pD3D12CreateDeviceAndSwapChain)
	{
		Msg("* DX12: failed to get address of D3D12CreateDeviceAndSwapChain");
		return false;
	}
	...
	D3D_FEATURE_LEVEL pFeatureLevels[] = { D3D_FEATURE_LEVEL_12_0 };
	D3D_FEATURE_LEVEL FeatureLevel;

	ID3D12Device*           pd3dDevice			= NULL;
	ID3D12Device*			pContext			= NULL;
	IDXGISwapChain*         pSwapChain			= NULL;

	/*hr = pD3D12CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, pFeatureLevels, 1,
		D3D12_SDK_VERSION, &sd, &pSwapChain, &pd3dDevice, &FeatureLevel, &pContext);

	if (FAILED(hr))
		Msg("* D3D12: device creation failed with hr=0x%08x", hr);
		*/
	if (pContext) pContext->Release();
	if (pSwapChain) pSwapChain->Release();
	if (pd3dDevice) pd3dDevice->Release();
