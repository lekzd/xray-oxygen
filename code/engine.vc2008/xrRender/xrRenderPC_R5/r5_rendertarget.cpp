...
#include "r5_blender_light_occq.h"
#include "r5_blender_light_mask.h"
#include "r5_blender_light_direct.h"
#include "r5_blender_light_point.h"
#include "r5_blender_light_spot.h"
#include "r5_blender_light_reflected.h"
#include "r5_blender_combine.h"
#include "r5_blender_bloom_build.h"
#include "r5_blender_luminance.h"
#include "r5_blender_ssao.h"
#include "dx12MinMaxSMBlender.h"
#include "dx12HDAOCSBlender.h"
#include "../xrRenderDX10/msaa/dxB2MSAABlender.h"
#include "../xrRenderDX10/DX10 Rain/dxB2RainBlender.h"
#include "r5_blender_fxaa.h"
...
//#TODO: Didn't fucking working shit. Really, who delete 2D texture struct in D3D12?
void CRenderTarget::u_setrt(const ref_rt& _1, const ref_rt& _2, const ref_rt& _3, ID3DDepthStencilView* zb)
{
	...
	else
	{
		...
		//HW.m_pAdapter->GetDesc1(&desc);

		D3D12_RESOURCE_DESC pTex = HW.pResource->GetDesc();

		dwWidth = pTex.Width;
		dwHeight = pTex.Height;
	}
   ...
   if( RImplementation.o.dx10_msaa )
	...
	// TONEMAP
	{
		...
		for (u32 it = 0; it < HW.Caps.iGPUNum * 2; it++) {
			...
			HW.pCommandList->ClearRenderTargetView(rt_LUM_pool[it]->pRT, ColorRGBA);
		}
		//#VERTVER to #GIPERION: fix render target view!
	...
	{
		u32		w = Device.dwWidth, h = Device.dwHeight;
		rt_ssao_temp.create(r2_RT_ssao_temp, w, h, D3DFMT_R16F, 1);
	...
	// Build textures
	{
		// Testure for async sreenshots
		{
			...
			//#NOTE: texture desc now is resource desc
			D3D_RESOURCE_DESC textureDesc = {};
			textureDesc.Format = DXGI_FORMAT_R8G8B8A8_SNORM;
			textureDesc.Width = Device.dwWidth;
			textureDesc.Height = Device.dwHeight;
			...
			HW.pDevice->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT),
				D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_COPY_DEST, NULL, its_must_be_render_target, NULL);
		...
		// Build material(s)
		{
			...
			textureDesc.Width = TEX_material_LdotN;
			textureDesc.Height = TEX_material_LdotH;
			textureDesc.DepthOrArraySize = TEX_material_Count;
			textureDesc.SampleDesc.Count = 1;
			textureDesc.SampleDesc.Quality = 0;
			textureDesc.MipLevels = 1;
		...
		if (1)
		{
			...
			//D3D_TEXTURE2D_DESC	desc;
			D3D_RESOURCE_DESC textureDesc = {};
			textureDesc.Width = TEX_jitter;
			textureDesc.Height = TEX_jitter;
			textureDesc.MipLevels = 1;
